...
const cartSlice = createSlice({
  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      const existingItem = state.items.find((i) => i.item.id === item.item.id && i.userId === item.userId);
      if (existingItem) {
...
export const { addToCart, removeFromCart, clearCart,increament,decreament } = cartSlice.actions;
